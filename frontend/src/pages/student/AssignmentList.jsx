...
const StudentAssignmentList = () => {
  ...
  const confirmedCount   = assignments.filter(a => getStatus(a) === 'submitted').length;
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${dueBadge.cls}`}>
                        {dueBadge.label}
                      </span>
                      {isSubmitted && (
                      ...
                      {isPending && (
                        <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full font-medium">⏳ Confirm Pending</span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                      ...
                      <span>By: {a.creator_name || a.creator?.name}</span>
                      {a.onedrive_link && (
                        <a href={a.onedrive_link} target="_blank" rel="noreferrer"
                          className="text-blue-500 hover:underline font-medium">🔗 Open Folder</a>
                      )}
                    </div>
                  </div>
